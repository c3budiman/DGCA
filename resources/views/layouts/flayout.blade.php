  ...
  <header id="header" class="header-scrolled">
    <div class="container-fluid">
      ...
      <div id="logo" class="pull-left">
        ...
        <a href="{{$link}}"><img src="{{ asset($logo)}}" alt="" title="" style="max-width: 200px;max-height: 70px;margin-top: -28%;margin-left: 10%;" /></a>
      </div>
          ...
          $menu = DB::table('frontmenu')->select('id','nama','method')->orderBy('id', 'asc')->get();
           ...
           @foreach ($menu as $mn)
             @if (DB::table($mn->method)->count() > 0)
               ...
               $submenunow = explode('/',Request::path());
               if (sizeof($submenunow > 0)) {
                 $menunow = $submenunow[0];
               }
               if (sizeof($submenunow) > 1) {
                $submenunow = $submenunow[1];
               }
                ...
                ?>
                  @if ($sub)
                      @if ($mn->method=="our_service")
                        ...
                        @endif
                      @else
                        @if ($menunow == $mn->method)
                           <li class="menu-has-children menu-active"><a href="{{$link}}/{{$mn->method}}">{{$mn->nama}}</a>
                        @else
               ...
               @foreach ($sub as $s)
                 ...
                 ?>
                 @if ($submenunow == $s->method)
                    <li><a style="color: #5EC1A4;" href="{{$link}}/{{$mn->method}}/{{$s->method}}">{{$s->nama}}</a></li>
                 @else
